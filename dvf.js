...
if (menuItems.length > 0) {
  menuItems.forEach((item) => {
    item.addEventListener("click", () => {
      ...
      // Masquer tous les conteneurs de jeux
      gameContainers.forEach((game) => game.classList.add("hidden"));

      // Afficher le jeu correspondant au bouton cliqué
...
// ==========================================================================
// GESTION CENTRALE DU FLUX D'ACTIVITÉ POUR L'ADMINISTRATEUR (SÉCURISÉ)
// ==========================================================================
function adminTriggerTransaction(username, typeTx, amountTx) {
  try {
    ...
    // Dispatch un événement pour forcer l'actualisation si l'admin est ouvert à côté
...
// ==========================================================================
// 3. CODE DE LA PAGE D'INSCRIPTION & CONNEXION (INDEX.HTML)
// ==========================================================================
...
// Détection automatique du réseau béninois
  ...
  phoneInput.addEventListener("input", () => {
    ...
    if (num.length >= 4 && num.startsWith("01")) {
      ...
      if (mtnPrefixes.includes(prefix)) {
        ...
        badge.innerHTML = '<i class="fa-solid fa-money-bill-wave"></i> MTN';
        badge.style.display = "block";
      } else if (moovPrefixes.includes(prefix)) {
        badge.classList.add("badge-moov");
        badge.innerHTML = '<i class="fa-solid fa-wallet"></i> Moov';
        badge.style.display = "block";
      } else if (celtiisPrefixes.includes(prefix)) {
        badge.classList.add("badge-celtiis");
        badge.innerHTML = '<i class="fa-solid fa-mobile-screen"></i> Celtiis';
        badge.style.display = "block";
      }
    }
  });
...
// --- FORMULAIRE D'INSCRIPTION (ENREGISTREMENT EN BASE DE DONNÉES LOCALES) ---
...
if (registerForm) {
  registerForm.addEventListener("submit", (e) => {
    ...
    const passwordInput = document.getElementById("reg-password")
      ...
      : "";
    // On prend le numéro de téléphone comme identifiant ou un champ pseudo si tu en as un
    ...
    // Récupérer la liste complète des comptes créés pour vérification
    let currentUsers =
      JSON.parse(localStorage.getItem("yb_registered_accounts")) || [];

    // Vérifier si le numéro existe déjà
    const userExists = currentUsers.some((u) => u.phone === phoneInput);
    if (userExists) {
      alert("Ce numéro de téléphone est déjà inscrit.");
      return;
    }

    // Sauvegarder le compte complet localement
    currentUsers.push({
    ...
    // Déclencher aussi l'apparition dans la console de l'Administrateur (partie A)
    if (typeof adminTriggerRegistration === "function") {
      adminTriggerRegistration(usernameInput, phoneInput);
    }

    alert(
      `Compte YeagerBet créé avec succès pour ${usernameInput} ! Connectez-vous.`,
    );

    // Basculer automatiquement sur la boîte de connexion
    if (registerBox) registerBox.classList.add("hidden");
    if (loginBox) loginBox.classList.remove("hidden");
    registerForm.reset();
  });
...
if (loginForm) {
  loginForm.addEventListener("submit", (e) => {
    ...
    if (validUser) {
      // AJOUT : Vérification du statut de blocage
...
// MOTEUR AUTOMATIQUE CYBERCRASH SPECIAL (STYLE MANGA)
// ==========================================================================
...
const historyBadgesEl = document.getElementById("crash-history-badges");

// Fonction Générateur de Crash (Algorithme d'instabilité)
// Fonction Générateur de Crash Réaliste (Sans limite maximale fixe)
function generateCrashPoint() {
  const rand = Math.random();

  // 1. Sécurité maison : 12% de chance de crash immédiat à 1.00x
  if (rand < 0.12) return 1.0;

  // 2. Formule mathématique de courbe de crash (Inverse Proportionnelle)
  // Elle permet d'avoir souvent des petits scores (1.1x - 3x)
  // mais laisse une chance réelle de monter à 10x, 50x, 100x+ de façon aléatoire.
  const alpha = 0.96; // Ajuste l'avantage de la maison (0.96 = 4% de marge pour le site)
  const crashPoint = alpha / (1 - rand);

  // On s'assure que le score ne descend jamais sous 1.01
  const finalScore = Math.max(1.01, crashPoint);

  return parseFloat(finalScore.toFixed(2));
}

// Jouer le son de défaite / bug
function playCrashSound() {
  ...
  if (sound) {
    ...
    sound
      .play()
      .catch((e) =>
        console.log("L'audio nécessite une interaction utilisateur d'abord."),
      );
  } else {
    // Option de secours : Bip système si aucun fichier audio n'est fourni
    const audioCtx = new (window.AudioContext || window.webkitAudioContext)();
    const oscillator = audioCtx.createOscillator();
    const gainNode = audioCtx.createGain();
    oscillator.type = "sawtooth"; // Son flippant/rétro
    oscillator.frequency.setValueAtTime(120, audioCtx.currentTime); // Basse fréquence
...
// Inscription du joueur pour la prochaine manche
if (btnCrashStart) {
  btnCrashStart.addEventListener("click", () => {
    ...
    btnCrashStart.style.opacity = "0.6";
...
if (btnCrashCashout) {
  btnCrashCashout.addEventListener("click", () => {
    ...
    crashStatusEl.innerText = `Sécurisé ! +${winnings} FCFA (${currentMultiplier}x)`;
    btnCrashCashout.classList.add("hidden");
  });
}

// BOUCLE PRINCIPALE COMMUNE ET AUTOMATIQUE
function startAutomatedCycle() {
  let countdown = nextGameTimer;

  // Phase de préparation (Le joueur peut analyser et parier)
  btnCrashStart.classList.remove("hidden");
  btnCrashStart.innerText = "Placer la mise";
  btnCrashStart.style.opacity = "1";
  btnCrashStart.disabled = false;
  btnCrashCashout.classList.add("hidden");

  crashGraphEl.className = "crash-graph";
  hasCashedOut = false;
  ...
  let countdownInterval = setInterval(() => {
    if (countdown <= 0) {
      ...
      crashMultiplierEl.innerText = countdown + "s";
      crashStatusEl.innerText = "Préparation de la faille dimensionnelle...";
      countdown--;
    }
  }, 1000);
}
...
function launchGame() {
  ...
  crashGraphEl.className = "crash-graph running";
  crashStatusEl.innerText = "Énergie stable...";

  if (isPlayerInGame) {
    btnCrashStart.classList.add("hidden");
    btnCrashCashout.classList.remove("hidden");
  } else {
    btnCrashStart.innerText = "MANCHE EN COURS...";
    btnCrashStart.disabled = true;
  }

  const targetCrashPoint = generateCrashPoint();
  console.log(
    `[YeagerBet Log] Le crash secret de cette manche est programmé à : ${targetCrashPoint}x`,
  );
  ...
  crashInterval = setInterval(() => {
    if (currentMultiplier >= targetCrashPoint) {
      ...
      crashGraphEl.className = "crash-graph crashed";
      crashMultiplierEl.innerText = "CRASHED";
      crashStatusEl.innerText = `Faille brisée à ${targetCrashPoint}x`;

      // Jouer le son étrange/flippant de défaite
      playCrashSound();

      // Enregistrer dans l'historique commun
      addCrashHistory(targetCrashPoint, isPlayerInGame && hasCashedOut);

      // Reset l'état joueur
      isPlayerInGame = false;

      // Relancer le compte à rebours automatique après 3 secondes d'affichage du crash
      setTimeout(() => {
        startAutomatedCycle();
      }, 1500);
    } else {
      // Accélération dynamique : plus le multiplicateur monte, plus les bonds sont grands !
      let increment = 0.01;
      if (currentMultiplier > 1.5) increment = 0.02;
      if (currentMultiplier > 5.0) increment = 0.08;
      if (currentMultiplier > 15.0) increment = 0.35;
      if (currentMultiplier > 40.0) increment = 1.2; // Passé 40x, ça s'envole très vite !
      ...
      if (currentMultiplier % 3 === 0) {
        crashStatusEl.innerText = "Surcharge de mana détectée...";
      }
      // Effet visuel de flash sur le conteneur du jeu en cas de perte
...
// Fonction historique
function addCrashHistory(score, won) {
  if (!historyBadgesEl) return;
  const badge = document.createElement("span");
  badge.className = `badge-hist ${won ? "green" : "red"}`;
  badge.innerText = score.toFixed(2) + "x";
  historyBadgesEl.insertBefore(badge, historyBadgesEl.firstChild);
  if (historyBadgesEl.children.length > 5) {
    historyBadgesEl.removeChild(historyBadgesEl.lastChild);
  }
}

// Lancement automatique au chargement du dashboard
...
// ==========================================================================
// 1. CHARGEMENT DE LA PAGE, AFFICHAGE ET DÉTECTION DU RÉSEAU
// ==========================================================================
document.addEventListener("DOMContentLoaded", () => {
  const activeUser = JSON.parse(localStorage.getItem("yb_active_user"));
  const navUserElement = document.getElementById("nav-username");
  ...
  // Affichage des identifiants (Nom + Numéro) en haut de la page
  if (activeUser && navUserElement) {
    navUserElement.innerHTML = `<i class="fa-solid fa-circle-user" style="color:#00e5ff;"></i> ${activeUser.username} <span style="color:#64748b; font-size:0.8rem; margin-left:5px;">(${activeUser.phone})</span>`;
  }

  // Charger le solde sauvegardé de CE joueur spécifique
  if (activeUser && userBalanceSpan) {
    let accounts =
      JSON.parse(localStorage.getItem("yb_registered_accounts")) || [];
    let currentAccount = accounts.find((acc) => acc.phone === activeUser.phone);

    if (currentAccount && currentAccount.balance !== undefined) {
      userBalanceSpan.innerText = currentAccount.balance;
    } else {
      userBalanceSpan.innerText = 0; // 0 F par défaut si nouveau compte
    }
  }

  // Détection du réseau au tap pour le Dépôt
  const depPhoneInput = document.getElementById("deposit-phone");
  if (depPhoneInput) {
    depPhoneInput.addEventListener("input", () => {
      const txtNet = document.getElementById("deposit-detected-network");
      if (activeUser && depPhoneInput.value.trim() === activeUser.phone) {
        txtNet.innerText = activeUser.network + " (Validé ✅)";
        txtNet.style.color = "#22c55e";
      } else {
        txtNet.innerText = "Numéro non conforme au compte ❌";
        txtNet.style.color = "#ef4444";
      }
    });
  }

  // Détection du réseau au tap pour le Retrait
  const witPhoneInput = document.getElementById("withdraw-phone");
  if (witPhoneInput) {
    witPhoneInput.addEventListener("input", () => {
      const txtNet = document.getElementById("withdraw-detected-network");
      if (activeUser && witPhoneInput.value.trim() === activeUser.phone) {
        txtNet.innerText = activeUser.network + " (Validé ✅)";
        txtNet.style.color = "#22c55e";
      } else {
        txtNet.innerText = "Numéro non conforme au compte ❌";
...
// ==========================================================================
// 2. GESTION DE L'OUVERTURE ET FERMETURE DES POP-UPS (MODALS)
// ==========================================================================
...
// Fermer en cliquant en dehors d'une modal
...
// ==========================================================================
// 3. FONCTIONS OUTILS (SAUVEGARDE ET HISTORIQUE ADMIN)
// ==========================================================================
// Permet de stocker durablement l'argent sur le bon compte
function saveUserBalance(phone, newBalance) {
  let accounts =
    JSON.parse(localStorage.getItem("yb_registered_accounts")) || [];
  let accountIndex = accounts.findIndex((acc) => acc.phone === phone);

  if (accountIndex !== -1) {
    accounts[accountIndex].balance = newBalance;
    localStorage.setItem("yb_registered_accounts", JSON.stringify(accounts));
  }
}

// Envoie les données vers le panneau d'administration
function adminTriggerTransaction(username, typeTx, amountTx) {
  let currentTx = JSON.parse(localStorage.getItem("yb_transactions")) || [];
  let now = new Date();
  let dateString =
...
// =======================================================
// MODULE PAIEMENT PAS À PAS ISOLÉ (YEAGERBET)
// =======================================================
(function () {
  try {
    const btnOuvrirDepot = document.getElementById("open-deposit-btn");
    const btnOuvrirRetrait = document.getElementById("open-withdraw-btn");
    const fenetreModale = document.getElementById("deposit-modal");
    const btnFermerFenetre = document.getElementById("close-modal-btn");

    // Récupération des deux sections spécifiques
    const sectionDepot = document.getElementById("section-depot");
    const sectionRetrait = document.getElementById("section-retrait");

    // Clic sur "+ Dépôt" : On montre le dépôt, on cache le retrait
    if (btnOuvrirDepot && fenetreModale) {
      btnOuvrirDepot.addEventListener("click", () => {
        sectionRetrait.style.display = "none";
        sectionDepot.style.display = "block";
        fenetreModale.style.setProperty("display", "flex", "important");
      });
    }

    // Clic sur "Retrait" : On montre le retrait, on cache le dépôt
    if (btnOuvrirRetrait && fenetreModale) {
      btnOuvrirRetrait.addEventListener("click", () => {
        sectionDepot.style.display = "none";
        sectionRetrait.style.display = "block";
        fenetreModale.style.setProperty("display", "flex", "important");
      });
    }

    if (btnFermerFenetre && fenetreModale) {
      btnFermerFenetre.addEventListener("click", () => {
        fenetreModale.style.setProperty("display", "none", "important");
      });
    }

    // Action : "Lancer le dépôt" (FedaPay Widget)
    const actionDepot = document.getElementById("btn-depot");
    if (actionDepot) {
      actionDepot.addEventListener("click", () => {
        const montant = parseInt(
          document.getElementById("depot-montant").value,
        );
        const telephone = document.getElementById("depot-phone").value.trim();

        if (!montant || montant < 100) {
          alert("Veuillez entrer un montant valide (Minimum 100 XOF).");
          return;
        }
        if (!telephone) {
          alert(
            "Veuillez entrer le numéro de téléphone pour le Push MoM/Moov.",
          );
          return;
        }

        FedaPay.init({ public_key: "pk_live__jP5onv5eIuDd0fTAABA6W_V" });

        FedaPay.open({
          amount: montant,
          currency: { iso: "XOF" },
          title: "Dépôt YeagerBet",
          description: "Rechargement de compte joueur",
          customer: {
            email: "joueur.yeagerbet@gmail.com",
            phone_number: { number: telephone, country: "BJ" },
          },
          onCombinedTriggered: function (response) {
            alert(
              "Veuillez confirmer le dépôt sur votre appareil initial en entrant votre code PIN de transaction.",
            );
          },
          onComplete: function (response) {
            if (response.status === "approved") {
              alert(`Dépôt réussi ! ${montant} XOF ont été ajoutés.`);
              if (fenetreModale)
                fenetreModale.style.setProperty("display", "none", "important");
            } else {
              alert("La transaction a été refusée ou annulée.");
            }
          },
        });
      });
    ...
    const actionRetrait = document.getElementById("btn-retrait");
    if (actionRetrait) {
      actionRetrait.addEventListener("click", () => {
        const montantRetrait = parseInt(
          document.getElementById("retrait-montant").value,
        );
        const telephoneRetrait = document
          .getElementById("retrait-phone")
          .value.trim();
        ...
        if (!telephoneRetrait) {
          alert("Veuillez indiquer le numéro de téléphone de réception.");
          return;
        }

        alert(
          `Demande de retrait enregistrée !\nMontant : ${montantRetrait} XOF\nLe cash sera transféré vers le numéro ${telephoneRetrait} après vérification.`,
        );
        if (fenetreModale)
          fenetreModale.style.setProperty("display", "none", "important");
      });
    ...
    console.error("Erreur d'initialisation du module Cash :", erreur);
  }
})();

// Fonction pour basculer le statut d'un joueur (Bloquer / Débloquer)
function toggleBlockUser(phone) {
  let users = JSON.parse(localStorage.getItem("yb_registered_accounts")) || [];
  let userIndex = users.findIndex((u) => u.phone === phone);

  if (userIndex !== -1) {
    // Si la propriété n'existe pas, on l'initialise à false, sinon on l'inverse
    users[userIndex].isBlocked = !users[userIndex].isBlocked;
    localStorage.setItem("yb_registered_accounts", JSON.stringify(users));

    // Si l'utilisateur est actuellement connecté et qu'on le bloque, on force sa déconnexion indirecte
    let activeUser = JSON.parse(localStorage.getItem("yb_active_user"));
    if (
      activeUser &&
      activeUser.phone === phone &&
      users[userIndex].isBlocked
    ) {
      // On met à jour son statut bloqué dans la session active pour que son app.js le déconnecte
      activeUser.isBlocked = true;
...
function loadAdminData() {
  const users =
    JSON.parse(localStorage.getItem("yb_registered_accounts")) || [];
  const transactions =
    JSON.parse(localStorage.getItem("yb_transactions")) || [];
  const notifications =
    JSON.parse(localStorage.getItem("yb_admin_notifications")) || [];

  let totalDeposits = 0;
  let totalGainsJoueurs = 0;
  let totalPertesJoueurs = 0;

  transactions.forEach((tx) => {
    if (tx.type === "depot") totalDeposits += tx.amount;
    if (tx.type === "gain") totalGainsJoueurs += tx.amount;
    if (tx.type === "perte") totalPertesJoueurs += tx.amount;
  });

  let comptePrincipal = totalPertesJoueurs - totalGainsJoueurs;

  // Mise à jour des compteurs statistiques
  if (document.getElementById("stat-count-users"))
    document.getElementById("stat-count-users").innerText = users.length;
  if (document.getElementById("stat-total-deposits"))
    document.getElementById("stat-total-deposits").innerText =
      totalDeposits.toLocaleString("fr-FR") + " F";
  if (document.getElementById("stat-gains-joueurs"))
    document.getElementById("stat-gains-joueurs").innerText =
      totalGainsJoueurs.toLocaleString("fr-FR") + " F";

  const mainAccEl = document.getElementById("stat-main-account");
  if (mainAccEl) {
    mainAccEl.innerText = comptePrincipal.toLocaleString("fr-FR") + " F";
    mainAccEl.className = comptePrincipal >= 0 ? "value green" : "value red";
  }

  // CORRECTION DU SÉLECTEUR : Liaison exacte avec ton HTML d'origine (admin-users-logs)
  const userContainer = document.getElementById("admin-users-logs");
  if (userContainer) {
    userContainer.innerHTML = "";
    if (users.length === 0) {
      userContainer.innerHTML =
        '<div style="color: #64748b; text-align:center; padding-top:20px;">Aucun compte enregistré pour le moment.</div>';
    } else {
      users.forEach((u) => {
        const item = document.createElement("div");
        ...
        let isBlocked = u.isBlocked || false;
        let btnText = isBlocked
          ? '<i class="fa-solid fa-unlock"></i> Débloquer'
          : '<i class="fa-solid fa-ban"></i> Bloquer';
        let btnColor = isBlocked ? "#22c55e" : "#ef4444";
        let statusBadge = isBlocked
          ? ' <span style="color:#ef4444; font-size:0.75rem; font-weight:bold;">[BLOQUÉ]</span>'
          : "";

        item.innerHTML = `
                            <div class="log-details">
                                <span class="badge badge-user"><i class="fa-solid fa-user-check"></i></span> 
                                Pseudo : <strong>${u.username}</strong> (${u.phone || "Pas de numéro"})${statusBadge}
                                <div style="color:#64748b; font-size:0.75rem; margin-top:2px;">Rejoint le : ${u.dateJoin || "En attente"}</div>
                            </div>
                            <div>
                                <button onclick="toggleBlockUser('${u.phone}')" style="background:${btnColor}; color:#fff; border:none; padding:6px 12px; border-radius:4px; cursor:pointer; font-size:0.8rem; font-weight:bold; display:flex; align-items:center; gap:5px; transition: 0.2s;">
                                    ${btnText}
                                </button>
                            </div>
                        `;
  ...
  // Liaison exacte avec ton conteneur financier d'origine (admin-financial-logs)
  const financialContainer = document.getElementById("admin-financial-logs");
  if (financialContainer) {
    financialContainer.innerHTML = "";
    if (transactions.length === 0) {
      financialContainer.innerHTML =
        '<div style="color: #64748b; text-align:center; padding-top:20px;">Aucun mouvement financier enregistré.</div>';
    } else {
      transactions.forEach((tx) => {
        const item = document.createElement("div");
        item.className = "log-item";
        let sign = tx.type === "gain" || tx.type === "depot" ? "+" : "-";
        let color =
          tx.type === "gain" || tx.type === "depot" ? "#22c55e" : "#ef4444";
        item.innerHTML = `<div>${tx.username} (${tx.type.toUpperCase()})</div><div style="color:${color}; font-weight:bold;">${sign} ${tx.amount} F</div>`;
        financialContainer.appendChild(item);
      });
    }
  }

  // Gestion de la grande boîte de notifications globales s'il y en a une
  const notifContainer = document.getElementById("admin-global-notifications");
  if (notifContainer) {
    if (notifications.length > 0) {
      notifContainer.innerHTML = "";
      notifications.forEach((n) => {
        const boxNotif = document.createElement("div");
        boxNotif.className = `notif-item notif-${n.type}`;
        boxNotif.innerHTML = `<div><i class="fa-solid fa-circle-info"></i> ${n.text}</div><div style="color:#64748b; font-size:0.8rem; margin-left:15px;">${n.time}</div>`;
        notifContainer.appendChild(boxNotif);
      });
    }
  }
}
